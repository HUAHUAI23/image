...
export function WeChatPayModal({ open, onOpenChange, onSuccess, config }: WeChatPayModalProps) {
  ...
  useEffect(() => {
    if (orderInfo?.codeUrl) {
      QRCode.toDataURL(orderInfo.codeUrl, {
        width: 280,
        margin: 2,
        color: {
          dark: '#000000',
      ...
      const pollOrder = async () => {
        try {
          const response = await fetch(
            `/api/wechat-pay/query-order?outTradeNo=${orderInfo.outTradeNo}`
          )
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px] p-0 gap-0 overflow-hidden border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-green-50/50 to-transparent dark:from-green-950/20 pointer-events-none" />

        {/* Header */}
        <div className="relative flex items-center justify-between px-6 py-5 border-b border-zinc-100 dark:border-zinc-800/50">
          <DialogTitle className="text-base font-semibold text-zinc-900 dark:text-zinc-100">微信支付</DialogTitle>
        </div>
